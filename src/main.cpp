...
int parseIntParameter(string parameterName, int minValue,
  ...
  if (pos == arguments.end()) {
    ...
    if ((parsedValue = stoi(value)) < minValue) {
      throw invalid_argument("'--" + parameterName + "' must be >= " + to_string(minValue));
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  if (arguments.size() == 0) {
    std::cout
        << "Usage: filter-designer --frequency 100 --samplingRate 48000\n";
    return 0;
  }

  try {
    const int cutoffFrequencyHz = parseFrequency(arguments);
    std::cout << "Cutoff Frequency " << cutoffFrequencyHz << "Hz\n";

    const int samplingRateHz = parseSamplingRate(arguments);
    std::cout << "Sampling Rate " << samplingRateHz << "Hz\n";

    auto rcGrid = new RCGrid(cutoffFrequencyHz);
    IIRCoefficients coefficients =
        rcGrid->calculateIIRFilterCoefficients(samplingRateHz);

    std::cout << "A=" << coefficients.a << "; B=" << coefficients.b << "\n";
