...
BOOST_AUTO_TEST_SUITE(LowPassRCCircuit_test)

BOOST_AUTO_TEST_CASE(constructor_test) {
    BOOST_REQUIRE_THROW(LowPassRCCircuit(-1, 100), invalid_argument);
    BOOST_REQUIRE_THROW(LowPassRCCircuit(100, -1), invalid_argument);
    BOOST_REQUIRE_THROW(LowPassRCCircuit(100, -1), invalid_argument);
    BOOST_REQUIRE_THROW(LowPassRCCircuit(100, 200), invalid_argument);
    BOOST_REQUIRE_NO_THROW(LowPassRCCircuit(99, 200));
}

void testCoefficients(int cutoffFrequency, int samplingRate) {
    auto circuit = LowPassRCCircuit(cutoffFrequency, samplingRate);
...
void testFrequencyResponse(int cutoffFrequency, int samplingRate) {
  cout << "IIR RC Circuit Response"
       << ", cutoff=" << cutoffFrequency << ", samplingRate=" << samplingRate
       << "\n";

  LowPassRCCircuit circuit = LowPassRCCircuit(cutoffFrequency, samplingRate);
